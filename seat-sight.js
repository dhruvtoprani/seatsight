if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  ...
  const toast = $('toast');
  const povScreen = $('povScreen');
  const povRoom = $('povRoom');
  const crosshair = $('crosshair');
  const previewTitle = $('previewTitle');
  const viewPill = $('viewPill');
  const selectedHeroSeat = $('selectedHeroSeat');
  const bestSeatBtn = $('bestSeatBtn');

  const seats = rows.flatMap((row, rowIndex) => Array.from({ length: 14 }, (_, index) => {
  ...
  function scoreSeat(seat) {
    ...
    const angle = Math.round(Math.atan2(Math.abs(seat.x), seat.rowIndex + 3) * 57.2958);
    const distance = `${Math.round((seat.rowIndex + 3.5) * 4.7)} ft`;
    let label = 'Balanced';
  ...
  function updatePreview(seat, meta) {
    ...
    povScreen.style.setProperty('--screen-w', `${78 - depth * 33 - Math.abs(side) * 9}%`);
    povScreen.style.setProperty('--screen-h', `${45 - depth * 18 - Math.abs(side) * 4}%`);
    povScreen.style.setProperty('--screen-angle', `${-side * 21}deg`);
    povScreen.style.left = `${50 - side * 10}%`;
    povScreen.style.top = `${48 + (depth - .5) * 4}%`;
  ...
  function renderDetails(seat, meta) {
    if (confirmed) {
      ...
      details.innerHTML = `
        ...
        <p class="muted">Great choice — ${meta.label.toLowerCase()} with a ${meta.grade.toLowerCase()} view score.</p>
        <div class="metrics">
      ...
      `;
      $('changeSeat').addEventListener('click', () => {
        confirmed = false;
        updateUI();
      });
      return;
